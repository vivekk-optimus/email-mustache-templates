/**
 * ReturnCancelledTpl.js
 * @description :: Email template will be used when a Return is cancelled.
 * 
...
module.exports = {
    subject: `{{portal_name}} Return {{return_id}} has been cancelled - ({{brand}})`,
    content: `
    <section class='email-header'>
        <article id='order-heading' style='font-size: 16px; padding: 10px'>Return ID {{return_id}} for {{customer}} has been cancelled.
        </article>
        <article style='padding: 5px 10px 5px 10px'>Cancelled by - {{user_cancelled}}
        </article>
        <article style='padding: 5px 10px 5px 10px'>Cancelled reason - {{cancelled_reason}}
        </article>
    </section>
